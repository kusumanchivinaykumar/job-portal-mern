...
const JobListing = () => {
    ...
    useEffect(() => {
        ...
        const matchesSearchLocation = job => searchFilter.location === "" || job.location.toLowerCase() === "" || job.location.toLowerCase().includes(searchFilter.location.toLowerCase())

        const newFilteredJobs = jobs.slice().reverse().filter(
            job => matchesCategory(job) && matchesLocation(job) && matchesTitle(job) && matchesSearchLocation(job) && matchesTitle(job) && matchesSearchLocation(job)
        )
    ...
    return (
        <div className='container 2xl:px-20 mx-auto flex flex-col lg:flex-row max-lg:space-y-8 py-8'>
            ...
            {/* SideBar */}
            <div className='w-full lg:w-1/4 px-4 bg-white max-h-[calc(100vh-100px)] overflow-y-auto sticky top-[100px]'>
